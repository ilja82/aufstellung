...
export class LineupSetupComponent implements OnInit, OnDestroy {
  ...
  startLineup(): void {
    ...
    const autoGeneratedName = this.generateDefaultName();
    const currentLineup = this.lineupService.currentLineup();

    if (currentLineup) {
      const savedLineup = this.savedLineupService.saveLineup(currentLineup, autoGeneratedName);
